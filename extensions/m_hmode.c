...
static int mo_hmode(struct Client *, struct Client *, int, const char **);

struct Message hmode_msgtab = {
	"HMODE", 0, 0, 0, MFLG_SLOW,
	{mg_unreg, mg_not_oper, mg_ignore, mg_ignore, mg_ignore, {mo_hmode, 3}}
};

mapi_clist_av1 hmode_clist[] = { &hmode_msgtab, NULL };

DECLARE_MODULE_AV1(hmode, NULL, NULL, hmode_clist, NULL, NULL, "$Revision: 3121 $");
 ...
 */
static int
mo_hmode(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	sendto_wallops_flags(UMODE_WALLOP, &me, 
			     "HMODE called for [%s] [%s] by %s!%s@%s",
			     parv[1], params, source_p->name, source_p->username, source_p->host);
	ilog(L_MAIN, "HMODE called for [%s] [%s] by %s",
	     parv[1], params, get_halfop_name(source_p));

	if(*chptr->chname != '&')
		sendto_server(NULL, NULL, NOCAPS, NOCAPS, 
			      ":%s WALLOPS :HMODE called for [%s] [%s] by %s!%s@%s",
	...
	set_channel_mode(client_p, source_p->servptr, chptr, msptr, 
			 parc - 2, parv + 2);
#else
	if (parc == 4 && !strcmp(parv[2], "+h") && !irccmp(parv[3], source_p->name))
	{
		/* Opping themselves */
		if (!wasonchannel)
		{
	...
	else
	{
		...
		{
			add_user_to_channel(chptr, source_p, CHFL_CHANHALFOP);
			msptr = find_channel_membership(chptr, source_p);
		}
