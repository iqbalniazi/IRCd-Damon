...
#include "channel.h"
#include "client.h"
#include "hash.h" // where to we define it? i mean decalre
#include "match.h"
#include "ircd.h"
#include "numeric.h"
#include "s_user.h"
...
static int mo_amode(struct Client *, struct Client *, int, const char **);

struct Message amode_msgtab = {
	"AMODE", 0, 0, 0, MFLG_SLOW,
	{mg_unreg, mg_not_oper, mg_ignore, mg_ignore, mg_ignore, {mo_amode, 3}}
};

mapi_clist_av1 amode_clist[] = { &amode_msgtab, NULL };

DECLARE_MODULE_AV1(amode, NULL, NULL, amode_clist, NULL, NULL, "$Revision: 3121 $");
 ...
 */
static int
mo_amode(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	/* admins only */
	if(!IsProtectedAdmin(source_p))
	{
		sendto_one(source_p, form_str(ERR_NOPRIVS), me.name, source_p->name, "protected");
		return 0;
	}
	...
	sendto_wallops_flags(UMODE_WALLOP, &me, 
			     "AMODE called for [%s] [%s] by %s!%s@%s",
			     parv[1], params, source_p->name, source_p->username, source_p->host);
	ilog(L_MAIN, "AMODE called for [%s] [%s] by %s",
	     parv[1], params, get_owner_name(source_p));

	if(*chptr->chname != '&')
		sendto_server(NULL, NULL, NOCAPS, NOCAPS, 
			      ":%s WALLOPS :AMODE called for [%s] [%s] by %s!%s@%s",
	...
	else
	{
		...
		{
			add_user_to_channel(chptr, source_p, CHFL_CHANPROTECT);
			msptr = find_channel_membership(chptr, source_p);
		}
