...
#define SM_ERR_NOOWNERS			0x00000002
#define SM_ERR_NOPROTECTEDS		0x00000004
#define SM_ERR_NOOPS            0x00000008	/* No chan ops */
#define SM_ERR_NOHALFOPS		0x00000010
#define SM_ERR_UNKNOWN          0x00000020
#define SM_ERR_RPL_C            0x00000040
#define SM_ERR_RPL_B            0x00000080
#define SM_ERR_RPL_E            0x00000100
#define SM_ERR_NOTONCHANNEL     0x00000200	/* Not on channel */
#define SM_ERR_RPL_I            0x00000400
#define SM_ERR_RPL_D            0x00000800
#define SM_ERR_NOPRIVS          0x00001000
#define SM_ERR_RPL_Q            0x00002000
#define SM_ERR_RPL_F            0x00004000
	...
	for(i = 0; i < 256; i++)
	{
		if( !(chmode_table[i].set_func == chm_ban) && 
			...
			!(chmode_table[i].set_func == chm_throttle) &&
						...
						!(chmode_table[i].set_func == chm_protected) &&
       ...
       const char **parv, int *errors, int dir, char c, long mode_type)
{
	...
	if(alevel != CHFL_CHANOWNER)
	{
		if(!(*errors & SM_ERR_NOOWNERS)) 
			sendto_one(source_p, form_str(ERR_CHANOPRIVSNEEDED),
				   me.name, source_p->name, chptr->chname);
		*errors |= SM_ERR_NOOWNERS;
		return;
	}
...
void // Here is the function you should copypasta  :O
chm_protected(struct Client *source_p, struct Channel *chptr,
       int alevel, int parc, int *parn,
       const char **parv, int *errors, int dir, char c, long mode_type)
{
	struct membership *mstptr;
	const char *protectednick;
	struct Client *targ_p;

	if(alevel != CHFL_CHANPROTECT)
	{
		if(!(*errors & SM_ERR_NOPROTECTEDS))
			sendto_one(source_p, form_str(ERR_CHANOPRIVSNEEDED),
				   me.name, source_p->name, chptr->chname);
		*errors |= SM_ERR_NOPROTECTEDS;
		return;
	}
	...
	protectednick = parv[(*parn)];
	(*parn)++;

	/* empty nick */
	if(EmptyString(protectednick))
	{
		sendto_one_numeric(source_p, ERR_NOSUCHNICK, form_str(ERR_NOSUCHNICK), "*");
		return;
	}

	if((targ_p = find_chasing(source_p, protectednick, NULL)) == NULL)
	{
		return;
	}
	...
	if(mstptr == NULL)
	{
		if(!(*errors & SM_ERR_NOTONCHANNEL) && MyClient(source_p))
			sendto_one_numeric(source_p, ERR_USERNOTINCHANNEL,
					   form_str(ERR_USERNOTINCHANNEL), protectednick, chptr->chname);
		*errors |= SM_ERR_NOTONCHANNEL;
		return;
	}
...
{
	struct membership *mstptr;
	const char *halfopnick; // here
	struct Client *targ_p;

	if(alevel != CHFL_CHANHALFOP)
	{
		if(!(*errors & SM_ERR_NOHALFOPS))
			sendto_one(source_p, form_str(ERR_CHANOPRIVSNEEDED),
				   me.name, source_p->name, chptr->chname);
		*errors |= SM_ERR_NOOPS;
		return;
	...
	/* empty nick */
	if(EmptyString(halfopnick)) // undeclared var, you must misnamed the declaration So it shou
	{
		sendto_one_numeric(source_p, ERR_NOSUCHNICK, form_str(ERR_NOSUCHNICK), "*");
		return;
	}
	  ...
	  const char **parv, int *errors, int dir, char c, long mode_type)
{
	...
	if(mstptr == NULL) // from here going down. 
	{
		if(!(*errors & SM_ERR_NOTONCHANNEL) && MyClient(source_p))
			sendto_one_numeric(source_p, ERR_USERNOTINCHANNEL,
					   form_str(ERR_USERNOTINCHANNEL), opnick, chptr->chname);
	...
	if(dir == MODE_ADD)
	{
		...
		mode_changes[mode_count].dir = MODE_ADD;
		mode_changes[mode_count].caps = 0;
		mode_changes[mode_count].nocaps = 0; //ok i think there is a syntaxc error here 
		mode_changes[mode_count].mems = ALL_MEMBERS;
		mode_changes[mode_count].id = targ_p->id;
		mode_changes[mode_count].arg = targ_p->name;
		mode_changes[mode_count++].client = targ_p;
/*
	...
	else
	{
		...
		mode_changes[mode_count].dir = MODE_DEL; // what do you think?
		mode_changes[mode_count].caps = 0;
		mode_changes[mode_count].nocaps = 0;
		mode_changes[mode_count].mems = ALL_MEMBERS;
		mode_changes[mode_count].id = targ_p->id;
		...
		mstptr->flags &= ~CHFL_CHANHALFOP; // wait wat does halfops does here?idk :s I probably messed up 
	}
} 
*/
 ...
 // and here, it's bad code I think wait lemme check
void
